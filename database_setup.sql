...
-- Drop all possible versions of get_user_inventory function
DO $$
DECLARE
    r RECORD;
BEGIN
    FOR r IN (
        ...
        FROM pg_proc p
        JOIN pg_namespace n ON p.pronamespace = n.oid
        WHERE p.proname = 'get_user_inventory' AND n.nspname = 'public'
    ) LOOP
        EXECUTE r.format;
    END LOOP;
END $$;
...
-- Function to get user inventory sorted by date (newest first)
CREATE FUNCTION public.get_user_inventory(
  p_telegram_id BIGINT
)
RETURNS TABLE (
  skin_name TEXT,
...
BEGIN
  ...
  SELECT 
    ...
    ui.unique_id,
    ui.created_at,
    COALESCE(ui.acquired_date, ui.created_at) as acquired_date
  FROM 
    public.user_inventory ui
  WHERE 
    ui.telegram_id = p_telegram_id
  ORDER BY 
    COALESCE(ui.acquired_date, ui.created_at) DESC,
    ui.skin_tier DESC,
    ui.skin_name ASC;
END;
$$;
...
GRANT EXECUTE ON FUNCTION public.get_user_inventory(BIGINT) TO authenticated;
...
GRANT EXECUTE ON FUNCTION public.get_user_inventory(BIGINT) TO anon;
...
COMMENT ON FUNCTION public.get_user_inventory IS 'Gets user inventory sorted by acquisition date (newest first)';
