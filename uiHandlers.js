...
function attachEventListeners() {
    ...
    if (tonConnectWalletButton) {
        tonConnectWalletButton.addEventListener('click', async () => {
            if (window.TonConnectWallet) {
                await window.TonConnectWallet.handleButtonClick();
            } else {
                console.error('[UI Handlers] TonConnectWallet module not loaded.');
                showToast('Wallet service is not available. Please refresh.', 'error');
            }
        });
    ...
    if (buyUcoinButtons.length > 0) {
        buyUcoinButtons.forEach(button => {
            button.addEventListener('click', async function() {
                ...
                if (window.TonConnectWallet) {
                    await window.TonConnectWallet.handleUcoinBuyClick(ucoins, tonAmount);
                } else {
                    console.error('[UI Handlers] TonConnectWallet module not loaded.');
                    showToast('Wallet service is not available. Please refresh.', 'error');
                }
    ...
    if (ucoinModalConnectBtn) {
        ucoinModalConnectBtn.addEventListener('click', async () => {
            if (window.TonConnectWallet) {
                await window.TonConnectWallet.handleModalConnectClick();
            } else {
                console.error('[UI Handlers] TonConnectWallet module not loaded.');
                showToast('Wallet service is not available. Please refresh.', 'error');
            }
